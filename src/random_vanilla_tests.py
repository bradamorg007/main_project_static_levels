    ...
    while not done:
        ...
        for i in range(SIM_SPEED):
            ...
            if change_level_flag:
                ...
                print('LEVEL ' + str(current_level_no) + ' COMPLETE: RANDOM VANILLA')
            ...
            if len(agents.not_sprites) == 0:
                ...
                if skip_ga == False:
                    ...
                    print('generation = %s level_generation = %s population size = %s level no = %s / %s' % (
                        internal_generation_count, level_generation_count, len(agents.not_sprites), current_level_no, len(levels)))
